...
public class Resolver implements
    ...
    private void resolveLocal(Expr expr, Token name) {
        for (int i = scopes.size() - 1; i >= 0; i--) {  // 从最近的作用域开始查找
            if (scopes.get(i).containsKey(name.lexeme)) { // 变量是否在当前作用域中？
                interpreter.resolve(expr, scopes.size() - 1 - i); // 计算变量的作用域深度
                return; // 找到后立即返回
            }
        }
    }
